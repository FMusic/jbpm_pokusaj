...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "jbpm_tecajna-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name jbpm_tecajna-service apps/jbpm_tecajna-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app jbpm_tecajna-service:1.0-SNAPSHOT
    oc expose svc/jbpm_tecajna-service
else

	echo "Launching the application locally..."
	pattern="jbpm_tecajna-service"
	files=( $pattern )
	cd ${files[0]}
