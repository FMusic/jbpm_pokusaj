...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "jbpm_tecajna-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name jbpm_tecajna-service apps/jbpm_tecajna-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app jbpm_tecajna-service:1.0-SNAPSHOT
		call oc expose svc/jbpm_tecajna-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd jbpm_tecajna-service
		cd target
